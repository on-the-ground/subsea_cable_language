 ...
 *   conflicts, and execution belong to the addressee the two declarations
 *   resolve, and an identifier neither declares is `UnknownPolicy` rather than
 *   a Scheduler default. Only interpretation inside one grounded leaf
 *   invocation ever moves to the Host; eligibility, the attempt lifecycle and
 *   scope outcomes stay with the Scheduler.
 *       @retry Goal1
