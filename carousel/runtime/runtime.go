...
func Start(cfg Config, unit *sema.Unit) (*Run, error) {
	...
	r.car = carousel.New(carousel.Config{Codebase: cfg.Codebase, Primitives: cfg.Host.Primitives(),
...
func (r *Run) Demand(id string) { r.car.Demand(id) }
...
func (r *Run) schedulerDemand() bool {
	...
	for _, o := range r.car.Occurrences() {
		if s := r.scopes[o.ID]; s != nil && s.deferred != nil && !s.State.terminal() {
			if ok, _ := r.structurallyReady(o); ok {
				r.trace.add(r.now, "DeductionFailureSurfaced", o.ID, s.deferred.Kind)
				r.settle(s, ScopeFailed, value.None, s.deferred, "deduction")
				any = true
			}
		}
	}
	for _, o := range r.car.Frontier() {
...
func (r *Run) handle(evs []carousel.Event) {
	for _, e := range evs {
		...
		case carousel.DeductionFailed:
			...
			if ok, _ := r.structurallyReady(r.car.Occurrence(e.Occ)); e.Reason == "prefetch" && !ok {
				s.deferred = e.Diag
