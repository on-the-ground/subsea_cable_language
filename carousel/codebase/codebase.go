...
import (
	...
	"github.com/on-the-ground/subsea_cable_language/carousel/expr"
...
const HashProfile = "poc-sha256-canon/0"
...
type Codebase struct {
	...
	byOcc     map[string]int
}

// New returns an empty Codebase.
func New() *Codebase {
	return &Codebase{artifacts: map[string]*Artifact{}, index: map[nameArity]string{}, byOcc: map[string]int{}}
}
...
// AppendDeduction commits a record. An occurrence commits at most once.
func (c *Codebase) AppendDeduction(r DeductionRecord) (DeductionRecord, error) {
	c.mu.Lock()
	defer c.mu.Unlock()
	if _, dup := c.byOcc[r.Occurrence]; dup {
		return r, fmt.Errorf("occurrence %s already committed", r.Occurrence)
	}
	r.Seq = len(c.ledger) + 1
	c.byOcc[r.Occurrence] = len(c.ledger)
	c.ledger = append(c.ledger, r)
	return r, nil
}

// GetDeduction returns the committed record for an occurrence.
func (c *Codebase) GetDeduction(occ string) (DeductionRecord, bool) {
	c.mu.Lock()
	defer c.mu.Unlock()
	i, ok := c.byOcc[occ]
	if !ok {
...
// Ledger returns a copy of all records in commit order.
...
func canonicalDef(def *sema.GoalDef, values *expr.Unit, erase bool, resolve func(*syntax.Name) string) string {
	...
	if erase {
		fmt.Fprintf(&b, "%s|/%d|", HashProfile, def.Arity)
	} else {
		fmt.Fprintf(&b, "%s|%s/%d|", HashProfile, def.Name, def.Arity)
	}
	...
	sort.Strings(names)
	for _, n := range names {
		b.WriteString("|" + n + "=" + syntax.Canonical(values.Values[n], erase, resolve))
	}
	return b.String()
}
