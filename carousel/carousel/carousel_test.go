...
import (
	...
	"github.com/on-the-ground/subsea_cable_language/carousel/carousel"
	"github.com/on-the-ground/subsea_cable_language/carousel/codebase"
	"github.com/on-the-ground/subsea_cable_language/carousel/expr"
	"github.com/on-the-ground/subsea_cable_language/carousel/host"
	"github.com/on-the-ground/subsea_cable_language/carousel/sema"
	"github.com/on-the-ground/subsea_cable_language/carousel/value"
)
...
func open(t *testing.T, cb *codebase.Codebase, src string, prefetch int) *fixture {
	...
	f.car = carousel.New(carousel.Config{Codebase: cb, Primitives: f.prim, Values: f.vals, Prefetch: prefetch})
...
func TestFailedDeductionIsAtomic(t *testing.T) {
	...
	if _, ok := cb.GetDeduction("r.d"); ok || len(b.Children) != 0 || len(cb.Ledger()) != 1 {
...
func TestPrimitiveSemanticsAreRequestedDuringDeduction(t *testing.T) {
	...
	f.step()
	rec := f.record("r.d")
	if f.prim.calls != 1 || rec.Arguments[0] != "n:2" || rec.PrimitiveProfile != "poc-rational/0" {
...
func TestValueBarrierBlocksPrefetch(t *testing.T) {
	...
	if _, ok := f.cb.GetDeduction("r.d.1"); ok {
	...
	f.step()
	if f.record("r.d.1").Arguments[0] != `s:"x"` {
...
func TestCarouselNeverEvaluatesLeaves(t *testing.T) {
	...
	cb.CommitUnit(u)
	h := host.NewScripted()
	car := carousel.New(carousel.Config{Codebase: cb, Primitives: h.Primitives(), Values: values{}, Prefetch: 10})
	car.OpenRoot(u)
	car.Demand("r")
	car.Replenish()
